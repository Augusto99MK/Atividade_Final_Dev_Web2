...
if ($autenticado) {
    ...
    // Verificar se a chave 'idProfessor' está definida no array $_SESSION['usuario']
    if (isset($_SESSION['usuario']['idProfessor'])) {
        // Obter o ID do professor da sessão
        $idProfessor = $_SESSION['usuario']['idProfessor'];

        // Realizar a consulta para buscar os cursos do professor
        require_once('../../conexao.php');
        $conexao = conectarBanco();

        try {
            $sql = "SELECT c.nomeCurso FROM curso c
                    INNER JOIN professor p ON c.idCurso = p.idCurso
                    WHERE p.idPessoa = ?";
